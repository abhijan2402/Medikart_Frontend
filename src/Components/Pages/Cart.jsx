...
export const Cart = () => {
    ...
    try {
      ...
      if (cashfreeRef.current) {
        cashfreeRef.current
          ...
          .catch((err) => {
            console.error("Payment failed or checkout error:", err);
            // Show failed message to user or handle gracefully
      ...
      if (selectedPayment === "cash") {
        ...
        if (selectedPayment === "wallet") {
          ...
          if (response.data === "You do not have enough money in wallet") {
            setErrorMessage(
              "Not enough balance in Wallet.Please top up your wallet or pick a different payment method."
            );
            setIsModalOpen(false);
          } else {
            handlePayment();
  ...
  const addAddress = async () => {
    try {
      ...
      const newAddress =
        `${addressName},${streetName},${buildingNumber},${floor},${appartment}`.replaceAll(
          " ",
          ""
        );
      const response = await axios.post(
  ...
  return (
    <>
      <Navigation pagetitle={"Shopping Cart"} />
      <Sidebar />
                        ...
                        <MDBCol lg="4" className="bg-grey">
                          <div className="p-5">
                            ...
                            <div className="mb-5">
                              ...
                              {!isValid && (
                                <p style={{ color: "red" }}>
                                  Invalid discount code. Please enter a valid
                                  code.
                                </p>
                              )}
                            </div>
                            ...
                            <div className="d-flex justify-content-between mb-1">
                              ...
                              <MDBTypography tag="h5">
                                ${" "}
                                {cart && (cart.discount * cart.bill).toFixed(2)}
                              </MDBTypography>
                            </div>
                            <div className="d-flex justify-content-between mb-1">
                              <MDBTypography
                              ...
                              <MDBTypography tag="h5">
                                ...
                                {cart &&
                                  cart.bill -
                                    (cart.discount * cart.bill).toFixed(2)}
                              </MDBTypography>
                            </div>
                        ...
                        <ModalContent>
                          ...
                          <ModalFooter>
                            <Button
                              colorScheme="teal"
                              onClick={() =>
                                handlePay(selectedAddress, selectedPayment)
                              }
                            >
                              Pay
                            </Button>
