...
export function MedicineList() {
  ...
  useEffect(() => {
    ...
    fetch("https://api.prabhatanvik.shop/admin/availableMedicines")
    ...
    fetch("https://api.prabhatanvik.shop/admin/MedicinalUse")
      .then((response) => response.json())
    ...
    const Prescription = async () => {
      try {
        const { data } = await axios.post(
          "https://api.prabhatanvik.shop/patient/Prescriptions",
          {},
          {
  ...
  const addToCart = async (medicine, quantity) => {
    try {
      const response = await axios.post(
        "https://api.prabhatanvik.shop/cart",
        {
  ...
  const getRelatedMeds = (currentMedicine) => {
    const relatedMeds = medicines.filter((medicine) =>
          ...
          <RadioGroup onChange={setValue} value={value}>
            <Stack direction="row">
              ...
              {Prescription.map((presc, index) => (
                <Button onClick={() => prescriptionMed(presc._id)}>
